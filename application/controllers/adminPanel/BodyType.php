...
class BodyType extends MY_Controller {
	...
	protected $name = 'bodyType';
    protected $title = 'body Type';
    private $table = "body_type";
    protected $redirect = "bodyType";
	...
	public function get()
    {
        $fetch_data = $this->main->make_datatables(admin('body_type_model'));
        $sr = $_POST['start'] + 1;
        $data = array();
        ...
        {  
            $sub_array = array();
            $sub_array[] = $sr;
            $sub_array[] = ucwords($row->body_type);
        ...
        $output = array(  
            ...
            "recordsFiltered"   => $this->main->get_filtered_data(admin('body_type_model')),
            "data"              => $data,
            $csrf_name          => $csrf_hash
        );
	...
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $data['url'] = $this->redirect;
            $data['data'] = $this->main->get($this->table, 'id, body_type', ['id' => d_id($id)]);
