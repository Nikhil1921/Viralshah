...
class Trim extends MY_Controller {
	...
	protected $name = 'trim';
    protected $title = 'trim';
    private $table = "trim";
    protected $redirect = "trim";
	...
	public function get()
    {
        $fetch_data = $this->main->make_datatables(admin('trim_model'));
        $sr = $_POST['start'] + 1;
        $data = array();
        ...
        $output = array(  
            ...
            "recordsFiltered"   => $this->main->get_filtered_data(admin('trim_model')),
            "data"              => $data,
            $csrf_name          => $csrf_hash
        );
    ...
    public function add()
	{
        ...
        else
        {
        	$post = [
                ...
                'variant_id' => d_id($this->input->post('variant_id')),
	...
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $data['url'] = $this->redirect;
            $data['data'] = $this->main->get($this->table, 'id, variant_id, trim, make_id', ['id' => d_id($id)]);
	        $data['makes'] = $this->main->getall('make', 'id, make', ['is_deleted' => 0]);
