...
class Make extends MY_Controller {
	...
	protected $name = 'make';
    protected $title = 'make';
    private $table = "make";
    protected $redirect = "make";
	...
	public function get()
    {
        $fetch_data = $this->main->make_datatables(admin('make_model'));
        $sr = $_POST['start'] + 1;
        $data = array();
        ...
        $output = array(  
            ...
            "recordsFiltered"   => $this->main->get_filtered_data(admin('make_model')),
            "data"              => $data,
            $csrf_name          => $csrf_hash
        );
	...
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $data['url'] = $this->redirect;
            $data['data'] = $this->main->get($this->table, 'id, make', ['id' => d_id($id)]);
