...
class VehicleClass extends MY_Controller {
	...
	protected $name = 'vehicleClass';
    protected $title = 'vehicle class';
    private $table = "vehicle_class";
    protected $redirect = "vehicleClass";
	...
	public function get()
    {
        $fetch_data = $this->main->make_datatables(admin('vehicle_class_model'));
        $sr = $_POST['start'] + 1;
        $data = array();
        ...
        $output = array(  
            ...
            "recordsFiltered"   => $this->main->get_filtered_data(admin('vehicle_class_model')),
            "data"              => $data,
            $csrf_name          => $csrf_hash
        );
	...
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $data['url'] = $this->redirect;
            $data['data'] = $this->main->get($this->table, 'id, class', ['id' => d_id($id)]);
