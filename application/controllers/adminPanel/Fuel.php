...
class Fuel extends MY_Controller {
	...
	protected $name = 'fuel';
    protected $title = 'fuel';
    private $table = "fuel";
    protected $redirect = "fuel";
	...
	public function get()
    {
        $fetch_data = $this->main->make_datatables(admin('fuel_model'));
        $sr = $_POST['start'] + 1;
        $data = array();
        ...
        $output = array(  
            ...
            "recordsFiltered"   => $this->main->get_filtered_data(admin('fuel_model')),
            "data"              => $data,
            $csrf_name          => $csrf_hash
        );
	...
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == FALSE)
        {
            ...
            $data['url'] = $this->redirect;
            $data['data'] = $this->main->get($this->table, 'id, fuel', ['id' => d_id($id)]);
